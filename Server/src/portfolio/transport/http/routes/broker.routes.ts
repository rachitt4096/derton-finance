...
import type { UpstoxAuthService } from '../../../broker/upstoxAuthService.js'
...
const readQuery = <T extends Record<string, string | undefined>>(request: FastifyRequest) => request.query as T

const readAuthorizedAdmin = async (
  request: FastifyRequest,
  reply: FastifyReply,
  authService: AuthService,
  config: AppConfig,
) => requireAdminUser(request, reply, authService, config.COOKIE_NAME)

export const registerBrokerRoutes = (
  app: FastifyInstance,
  authService: AuthService,
  upstoxAuthService: UpstoxAuthService,
  brokerCredentialStore: BrokerCredentialStore,
