...
const optionalTrimmedUrl = (fallback: string) =>
  z.preprocess((value) => {
...
export const parseConfig = (env: NodeJS.ProcessEnv): AppConfig => {
  ...
  if (invalidOrigins.length) {
    throw new Error(
      `Invalid server environment: APP_ORIGIN contains invalid URL values: ${invalidOrigins.join(', ')}`,
    )
  }
  ...
  if (isProduction) {
    ...
    if (insecureOrigins.length) {
      throw new Error(
        `Invalid server environment: APP_ORIGIN must use HTTPS and cannot point to localhost/loopback in production: ${insecureOrigins.join(', ')}`,
      )
    }
  }
  ...
  if (
    ...
    throw new Error(
      'Invalid server environment: SEED_ADMIN_PASSWORD is using the default value. Set a strong password for production.',
    )
  }
  ...
  if (isProduction && parsed.data.BROKER_MODE === 'upstox') {
    ...
    if (requiredBrokerFields.length) {
      throw new Error(
        `Invalid server environment: missing required Upstox production settings: ${requiredBrokerFields.map(([key]) => key).join(', ')}`,
      )
    }
  }
  ...
  if (alertsEnabled && !hasSlackChannel && !hasEmailWebhookChannel && !hasTwilioChannel) {
    throw new Error(
      'Invalid server environment: ALERTS_ENABLED=true requires at least one alert channel (Slack webhook, email webhook, or Twilio WhatsApp).',
    )
  }

  if (hasAnyTwilioField && !hasTwilioChannel) {
    throw new Error(
      'Invalid server environment: Twilio WhatsApp alert config is incomplete. Set ALERT_WHATSAPP_TWILIO_ACCOUNT_SID, ALERT_WHATSAPP_TWILIO_AUTH_TOKEN, ALERT_WHATSAPP_TWILIO_FROM, and ALERT_WHATSAPP_TWILIO_TO.',
    )
  }
