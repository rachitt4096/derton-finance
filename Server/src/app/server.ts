...
const toError = (error: unknown, fallback = 'Unexpected error') =>
...
const bootstrap = async () => {
  ...
  const upstoxQuoteService = new UpstoxQuoteService(
    config,
    instrumentService,
  ...
  const instrumentSyncInterval =
    config.UPSTOX_INSTRUMENTS_URL
      ? setInterval(() => {
