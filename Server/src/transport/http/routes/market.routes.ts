...
const MIN_USABLE_INTRADAY_CANDLES: Record<string, number> = {
  '1m': 60,
  '5m': 18,
  '15m': 6,
  '1h': 2,
...
const isSparseDatedIntradayCache = (interval: string, candles: CandlePoint[]) => {
  const minimum = MIN_USABLE_INTRADAY_CANDLES[interval]
  return Number.isFinite(minimum) && candles.length > 0 && candles.length < minimum
}

const isIntradayInterval = (interval: string) => interval !== '1d'

export const appendNewerCandles = (baseCandles: CandlePoint[], liveCandles: CandlePoint[]) => {
