...
export interface StoredBrokerCredential {
...
export class BrokerCredentialStore {
  ...
  async get(provider: BrokerProvider): Promise<StoredBrokerCredential | null> {
  ...
  async clear(provider: BrokerProvider) {
    await this.pool.query('delete from broker_credentials where provider = $1', [provider])
  }

  async resolveAccessToken(provider: BrokerProvider, envToken = '') {
