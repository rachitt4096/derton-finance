import type { HistoryInterval } from './upstoxHistoryService.js'
...
export const compareDate = (left: string, right: string) =>
  new Date(`${left}T00:00:00Z`).getTime() - new Date(`${right}T00:00:00Z`).getTime()

export const getChunkDays = (interval: HistoryInterval) => {
  switch (interval) {
    case '1m':
    case '5m':
    case '15m':
...
export const buildHistoryChunks = (fromDate: string, toDate: string, interval: HistoryInterval) => {
