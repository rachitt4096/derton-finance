...
import type { BrokerStatusSnapshot, BrokerTick, CandlePoint, LiveMarketQuote } from '../lib/contracts.js'
import { TickBuffer } from './tickBuffer.js'
import type { UpstoxQuoteService } from './upstoxQuoteService.js'
import { isNseTradingSessionOpen, normalizeBrokerStatusForSession } from './session.js'

const intervalToMs = (interval: '1m' | '5m' | '15m' | '1h' | '1d') =>
  ({
    '1m': 60_000,
    '5m': 5 * 60_000,
    '15m': 15 * 60_000,
...
const STORED_CANDLE_INTERVALS = ['1m', '5m', '15m', '1h', '1d'] as const

type CandleInterval = (typeof STORED_CANDLE_INTERVALS)[number]
type CandleSource = 'broker' | 'provider'

type StoredCandleInput = {
  symbol: string
...
const toBucketStartMs = (timestampMs: number, interval: CandleInterval) =>
  Math.floor(timestampMs / intervalToMs(interval)) * intervalToMs(interval)

const toBucketEndMs = (bucketStartMs: number, interval: CandleInterval) =>
  bucketStartMs + intervalToMs(interval) - 1

const aggregateTicksToCandles = (ticks: BrokerTick[], interval: CandleInterval): StoredCandleInput[] => {
  const buckets = new Map<string, StoredCandleInput>()
...
const normalizeHistoricalCandles = (
  ...
  candles
    ...
    .filter((candle): candle is StoredCandleInput => Boolean(candle))

export class MarketRuntime {
  private readonly tickBuffer = new TickBuffer()
  private readonly liveQuotes = new Map<string, LiveMarketQuote>()
  private readonly consumerSymbols = new Map<string, Set<string>>()
  private readonly statusListeners = new Set<(status: BrokerStatusSnapshot) => void>()
  private readonly subscribedSymbols = new Set<string>()
  private flushInterval: NodeJS.Timeout | null = null
  private cleanupInterval: NodeJS.Timeout | null = null
  private watchdogInterval: NodeJS.Timeout | null = null
  private brokerRestartInFlight = false
  private brokerRestartAttempts = 0
  private lastBrokerRestartAt = 0
  ...
  constructor(
    ...
    private readonly quoteService?: UpstoxQuoteService,
  ...
  getLatestQuotes(symbols?: string[]) {
    ...
    return Object.fromEntries(
      activeSymbols
        .map((symbol) => [symbol, this.liveQuotes.get(symbol)] as const)
        .filter((entry): entry is [string, LiveMarketQuote] => Boolean(entry[1])),
    )
  }
  ...
  async setConsumerSymbols(consumerId: string, symbols: string[]) {
    this.consumerSymbols.set(consumerId, new Set(symbols))
    await this.syncSubscriptionsSafe()
  }

  async clearConsumerSymbols(consumerId: string) {
  ...
  private async getStoredCandles(
    symbol: string,
    days: number,
    interval: CandleInterval,
    options: { date?: string } = {},
  ...
  private async getCandlesFromTicks(
    symbol: string,
    days: number,
    interval: CandleInterval,
    options: { date?: string } = {},
  ...
  private onTick(tick: BrokerTick) {
  ...
  private async flushPendingTicks() {
  ...
  private async syncSubscriptionsSafe() {
    try {
  ...
  private getLatestQuoteMap(symbols: string[]) {
    return this.getLatestQuotes(symbols)
  }

  private createEmptyQuote(symbol: string): LiveMarketQuote {
    return {
      symbol,
      companyName: null,
      exchange: null,
  ...
  private mergeQuote(symbol: string, patch: Partial<LiveMarketQuote>) {
  ...
  private async hydrateQuotes(symbols: string[]) {
  ...
  private async cleanupExpiredMarketData() {
    try {
      await this.pool.query(
        `
  ...
  private async insertTickChunk(ticks: BrokerTick[]) {
  ...
  private async upsertIncrementalCandles(candles: StoredCandleInput[]) {
    if (!candles.length) {
      return
    }
  ...
  private async replaceCandles(candles: StoredCandleInput[]) {
    if (!candles.length) {
      return
    }
  ...
  private toErrorMessage(error: unknown, fallback = 'Unknown runtime error') {
    return error instanceof Error ? error.message : fallback
  }

  private async watchBrokerHealth() {
    ...
    const hasRecoverableOfflineError =
      !this.status.error || !/missing upstox_access_token/i.test(this.status.error)
    ...
    const stalledFeed =
      (this.status.status === 'connecting' || this.status.status === 'degraded') &&
      tickAgeMs > STALE_TICK_THRESHOLD_MS * 2
    const offlineWithDemand =
    ...
    const backoffMs = Math.min(
      BROKER_RESTART_BACKOFF_BASE_MS * 2 ** Math.min(this.brokerRestartAttempts, 5),
