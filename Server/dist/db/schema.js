...
export const purgeExpiredSessions = async (pool) => {
    await pool.query('delete from sessions where expires_at <= now()');
};
export const purgeMarketHistory = async (pool, retentionDays) => {
    await pool.query(`
      delete from market_ticks
      where recorded_at < now() - ($1::text || ' days')::interval
    `, [retentionDays]);
};
export const purgeStoredCandles = async (pool, retentionDays) => {
    await pool.query(`
