...
export class AdminService {
    ...
    buildManagedUserEmail(username) {
    ...
    async getOverview() {
        const [users, sessions, instruments, ticks, watchlists] = await Promise.all([
          ...
          from users
        `),
            this.pool.query('select count(*)::text as active_sessions from sessions where expires_at > now()'),
            this.pool.query('select count(*)::text as instrument_count from instruments'),
            this.pool.query(`
          select
          ...
          from market_ticks
        `),
            this.pool.query('select count(*)::text as watchlist_count from watchlists'),
        ]);
    ...
    async createUser(actorUserId, input) {
    ...
    async resetPassword(actorUserId, userId, password) {
        const user = await this.getUserRow(userId);
        if (!user) {
            throw new AdminServiceError('User not found.', 404);
        }
    ...
    async revokeSessions(actorUserId, userId) {
        const user = await this.getUserRow(userId);
        if (!user) {
            throw new AdminServiceError('User not found.', 404);
        }
    ...
    async ensureAnotherActiveAdminExists(excludedUserId) {
        const result = await this.pool.query(`
    ...
    async writeAudit(actorUserId, action, entityType, entityId, payload) {
        await this.pool.query(`
