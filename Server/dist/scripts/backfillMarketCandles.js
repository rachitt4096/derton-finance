...
const parseOptions = () => {
    ...
    const interval = (parseArgValue('interval') || '1m');
...
const normalizeHistoricalCandles = (symbol, interval, candles) => candles
    .map((candle) => {
...
const replaceCandles = async (connectionString, rows) => {
    if (!rows.length) {
        return;
    }
    const values = [];
    const sqlRows = rows
        .map((row, index) => {
        const base = index * 11;
        values.push(row.symbol, row.interval, row.bucketStart, row.firstTradeAt, row.lastTradeAt, row.open, row.high, row.low, row.close, row.volume, row.source);
...
const main = async () => {
    ...
    try {
        ...
        console.log(`Starting candle backfill for ${options.symbols.join(', ')} from ${options.fromDate} to ${options.toDate} at ${options.interval} in ${chunks.length} chunk(s).`);
        for (const symbol of options.symbols) {
            ...
            for (const chunk of chunks) {
                ...
                console.log(`[${symbol}] ${chunk.fromDate} -> ${chunk.toDate}: fetched ${candles.length} candle(s), stored ${normalized.length}.`);
            }
