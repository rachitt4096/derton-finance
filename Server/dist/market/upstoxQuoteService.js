...
const normalizeDepth = (rows) => (rows ?? []).map((row) => ({
    quantity: toNumber(row.quantity) ?? 0,
    price: toNumber(row.price) ?? 0,
    orders: toNumber(row.orders) ?? 0,
}));
const callSdk = (runner) => new Promise((resolve, reject) => {
    runner((error, data) => {
        if (error) {
            reject(error);
            return;
...
export class UpstoxQuoteService {
    ...
    credentialStore;
    historyService;
    constructor(config, instrumentService, credentialStore, historyService) {
        this.config = config;
    ...
    async getQuotes(symbols) {
        ...
        const response = await callSdk((callback) => {
            quoteApi.getFullMarketQuote(instrumentKeys, '2.0', (error, data) => callback(error, data));
        });
        const rawData = response?.data ?? {};
        const shouldBackfillClosedSessionLtp = !isNseTradingSessionOpen();
        const minuteCloseBySymbol = new Map();
        if (shouldBackfillClosedSessionLtp) {
            const minuteCloses = await Promise.all(instruments.map(async (instrument) => {
                try {
                    return [instrument.symbol, await this.historyService.getLatestMinuteClose(instrument.symbol, 7)];
                }
                catch {
                    return [instrument.symbol, null];
                }
            }));
        ...
        const yearRanges = await Promise.all(instruments.map(async (instrument) => {
            try {
        ...
        return instruments.map((instrument, index) => {
            const raw = Object.values(rawData).find((item) => item?.instrument_token === instrument.instrumentKey ||
            ...
            const previousClose = pickNumber(raw?.ltpc?.cp, apiLastPrice !== null && rawNetChange !== null ? apiLastPrice - rawNetChange : null, raw?.ohlc?.close);
